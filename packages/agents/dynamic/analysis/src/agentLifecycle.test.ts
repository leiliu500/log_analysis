...
test('the inactivity timeout is per-app, sourced from transaction.md (SCP 30m, apiflc 10m)', async () => {
  // Two apps sharing the generic protocol but pointing at their real specs: SCP states a
  // 30-minute inactivity timeout, apiflc a 10-minute one. An agent idle for 15 minutes
  ...
  const idle15m = (application: string): Agent => ({
    messageId: application, application, status: 'awaiting', active: true, waitingFor: 'RESPONSE',
    phases: ['REQUEST', 'ACK', 'RESPONSE'], phaseTs: { REQUEST: NOW - 15 * 60_000 },
    spawnedAt: NOW - 15 * 60_000, updatedAt: NOW - 15 * 60_000,
  });

  // Fallback (opts.timeoutMs) is a large 60m so ONLY the per-app spec can trip apiflc.
  const r = await stepAgentsDynamic([], [idle15m('scp'), idle15m('apiflc')], { now: NOW, timeoutMs: 60 * 60_000, registry: perApp });
  assert.equal(r.agents.get('scp')!.status, 'awaiting', 'SCP (30m) still active after 15m');
  assert.equal(r.agents.get('apiflc')!.status, 'error', 'apiflc (10m) timed out after 15m');
});
