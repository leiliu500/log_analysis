...
/** Per-source "cloudwatch:3 · splunk:0" breakdown. */
...
export function ScheduleTab({ runs }: { runs: PollerRun[] }) {
  const lastScheduled = runs.find((r) => r.trigger === 'schedule');
  const totalParsed = runs.reduce(
    (n, r) => n + Object.values(r.bySource).reduce((m, s) => m + s.parsed, 0),
    0,
  );
    ...
    <section>
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        <span className="text-xs text-slate-500">
          EventBridge triggers the ingestion poller every ~5 min (parse · detect anomalies ·
          reason · advance the request/ack/response agent lifecycle). “Analyze now” shows as a{' '}
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <div className="card text-center">
          <div className="text-2xl font-semibold text-white">
            {runs.reduce((n, r) => n + r.findings, 0)}
          </div>
          ...
          <table className="w-full text-left text-xs">
            <thead className="text-slate-500">
              <tr className="border-b border-edge">
                ...
                <th className="px-3 py-2">parsed (by source)</th>
            ...
            <tbody className="font-mono text-slate-300">
              {runs.map((r) => (
                <tr key={r.id} className="border-b border-edge/50">
                  ...
                  <td className="px-3 py-1.5 font-sans text-slate-400">{sourceBreakdown(r)}</td>
                  <td className="px-3 py-1.5">
                    {r.agents.spawned}/{r.agents.advanced}/{r.agents.closed}
                  </td>
                  <td className={`px-3 py-1.5 ${r.findings ? 'text-amber-300' : ''}`}>{r.findings}</td>
